...
public class SpaceInvadersGUI extends Application {
	...
	public void start(Stage stage) throws Exception {
		...
		Bullet plyrBullet = new Bullet((int) spaceship.getLayoutX(), (int) spaceship.getLayoutY(),
				-1 * (int) bulletSpeed);
		...
		AnimationTimer timer = new AnimationTimer() {
			...
			public void handle(long arg0) {
				...
				if (playerBullet != null) {
					if (playerBullet.getLayoutY() < -1 * playerBullet.getFitHeight() && fireBullet
							|| bulletHitSomething(playerBullet)) {
					...
					playerBullet.setFitWidth(10);
					playerBullet.setFitHeight(25);
					playerBullet.setLayoutX(
							spaceship.getLayoutX() + spaceship.getFitWidth() / 2 - playerBullet.getFitWidth() / 2);
	...
	private boolean bulletHitSomething(ImageView bullet) {
		for (Shield shield : shields) {
			if (playerBullet != null
					&& playerBullet.getLayoutY() <= shield.getY() + shield.getImageView().getFitHeight()
					&& playerBullet.getLayoutX() >= shield.getImageView().getLayoutX() - playerBullet.getFitWidth()
					&& playerBullet.getLayoutX() <= shield.getImageView().getLayoutX()
							+ shield.getImageView().getFitWidth()
					&& shield.getHealth() > 0) {
				shield.hit();
				return true;
			}
